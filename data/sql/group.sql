...
alter table
  `group`
add
  column notice varchar(400) not null default '';
...
ALTER TABLE
  `group_member`
ADD
  COLUMN forbidden_expir_time integer NOT NULL DEFAULT 0 COMMENT '群成员禁言时长';

-- +migrate Up
ALTER TABLE
  `group`
ADD
  COLUMN avatar VARCHAR(255) NOT NULL DEFAULT '' COMMENT '群头像';
...
ALTER TABLE
  `group`
ADD
  COLUMN is_upload_avatar smallint not null DEFAULT 0 COMMENT '群头像是否已经被用户上传';

-- +migrate Up
ALTER TABLE
  `group`
ADD
  COLUMN group_type smallint not null DEFAULT 0 COMMENT '群类型 0.普通群 1.超大群';